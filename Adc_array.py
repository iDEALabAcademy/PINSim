...
class AdcArray(Component.Component):
    ...
    def __init__(self, name, model):
        ...
        if Network.type == "CNN":
            self.total_delay_in_compute = self.operation_delay_in_compute * (Network.calculate_output_height(Hardware.pixel_array_height) * (Network.kernel_width - Network.stride + 1)  * Network.kernel_number // Hardware.parallelism_level)
        else:
            self.total_delay_in_compute = self.operation_delay_in_compute * Network.calculate_output_height(Hardware.pixel_array_height)  // Hardware.parallelism_level
